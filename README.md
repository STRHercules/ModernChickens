...
>
> If `item_texture` is supplied the chicken item and JEI icons prefer that sprite. When the referenced texture cannot be loaded a warning is logged and the item temporarily reverts to the tinted fallback icon; otherwise tinting stays disabled so the art is rendered exactly as authored.
>
