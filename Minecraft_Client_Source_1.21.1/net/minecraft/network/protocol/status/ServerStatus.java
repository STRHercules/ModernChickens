...
public record ServerStatus(Component description, Optional<ServerStatus.Players> players, Optional<ServerStatus.Version> version, Optional<ServerStatus.Favicon> favicon, boolean enforcesSecureChat) {
   ...
   public static record Players(int max, int online, List<GameProfile> sample) {
      ...
      public static final Codec<ServerStatus.Players> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(Codec.INT.fieldOf("max").forGetter(ServerStatus.Players::max), Codec.INT.fieldOf("online").forGetter(ServerStatus.Players::online), PROFILE_CODEC.listOf().lenientOptionalFieldOf("sample", List.of()).forGetter(ServerStatus.Players::sample)).apply(var0, ServerStatus.Players::new);
      });

      public Players(int param1, int param2, List<GameProfile> param3) {
         super();
         this.max = var1;
         this.online = var2;
         this.sample = var3;
