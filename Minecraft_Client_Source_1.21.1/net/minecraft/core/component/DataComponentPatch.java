...
public final class DataComponentPatch {
   ...
   static {
      CODEC = Codec.dispatchedMap(DataComponentPatch.PatchKey.CODEC, DataComponentPatch.PatchKey::valueCodec).xmap((var0) -> {
         if (var0.isEmpty()) {
            return EMPTY;
         } else {
      ...
      STREAM_CODEC = new StreamCodec<RegistryFriendlyByteBuf, DataComponentPatch>() {
         ...
         public void encode(RegistryFriendlyByteBuf var1, DataComponentPatch var2) {
            if (var2.isEmpty()) {
               ...
               it.unimi.dsi.fastutil.objects.Reference2ObjectMap.Entry var6;
               while(var5.hasNext()) {
                  var6 = (it.unimi.dsi.fastutil.objects.Reference2ObjectMap.Entry)var5.next();
   ...
   public static record SplitResult(DataComponentMap added, Set<DataComponentType<?>> removed) {
      public static final DataComponentPatch.SplitResult EMPTY;

      public SplitResult(DataComponentMap param1, Set<DataComponentType<?>> param2) {
         super();
         this.added = var1;
         this.removed = var2;
      }
      ...
      static {
         EMPTY = new DataComponentPatch.SplitResult(DataComponentMap.EMPTY, Set.of());
      }
   }

   static record PatchKey(DataComponentType<?> type, boolean removed) {
      public static final Codec<DataComponentPatch.PatchKey> CODEC;

      PatchKey(DataComponentType<?> param1, boolean param2) {
         super();
         this.type = var1;
         this.removed = var2;
      }
