...
public class ReturnCommand {
   ...
   public static <T extends ExecutionCommandSource<T>> void register(CommandDispatcher<T> var0) {
      ...
      })).then(RequiredArgumentBuilder.argument("value", IntegerArgumentType.integer()).executes(new ReturnCommand.ReturnValueCustomExecutor()))).then(LiteralArgumentBuilder.literal("fail").executes(new ReturnCommand.ReturnFailCustomExecutor()))).then(LiteralArgumentBuilder.literal("run").forward(var0.getRoot(), new ReturnCommand.ReturnFromCommandCustomModifier(), false)));
   }
