...
public class ApplyBonusCount extends LootItemConditionalFunction {
   ...
   private static final Codec<ApplyBonusCount.FormulaType> FORMULA_TYPE_CODEC;
   private static final MapCodec<ApplyBonusCount.Formula> FORMULA_CODEC;
   ...
   static {
      FORMULAS = (Map)Stream.of(ApplyBonusCount.BinomialWithBonusCount.TYPE, ApplyBonusCount.OreDrops.TYPE, ApplyBonusCount.UniformBonusCount.TYPE).collect(Collectors.toMap(ApplyBonusCount.FormulaType::id, Function.identity()));
      ...
      FORMULA_CODEC = ExtraCodecs.dispatchOptionalValue("formula", "parameters", FORMULA_TYPE_CODEC, ApplyBonusCount.Formula::getType, ApplyBonusCount.FormulaType::codec);
   ...
   static record UniformBonusCount(int bonusMultiplier) implements ApplyBonusCount.Formula {
      public static final Codec<ApplyBonusCount.UniformBonusCount> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(Codec.INT.fieldOf("bonusMultiplier").forGetter(ApplyBonusCount.UniformBonusCount::bonusMultiplier)).apply(var0, ApplyBonusCount.UniformBonusCount::new);
      });
      public static final ApplyBonusCount.FormulaType TYPE;

      UniformBonusCount(int param1) {
         super();
      ...
      static {
         TYPE = new ApplyBonusCount.FormulaType(ResourceLocation.withDefaultNamespace("uniform_bonus_count"), CODEC);
      }
   }
   ...
   private static record OreDrops() implements ApplyBonusCount.Formula {
      ...
      static {
         TYPE = new ApplyBonusCount.FormulaType(ResourceLocation.withDefaultNamespace("ore_drops"), CODEC);
      }
   }

   private static record BinomialWithBonusCount(int extraRounds, float probability) implements ApplyBonusCount.Formula {
      private static final Codec<ApplyBonusCount.BinomialWithBonusCount> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(Codec.INT.fieldOf("extra").forGetter(ApplyBonusCount.BinomialWithBonusCount::extraRounds), Codec.FLOAT.fieldOf("probability").forGetter(ApplyBonusCount.BinomialWithBonusCount::probability)).apply(var0, ApplyBonusCount.BinomialWithBonusCount::new);
      });
      public static final ApplyBonusCount.FormulaType TYPE;

      BinomialWithBonusCount(int param1, float param2) {
         super();
      ...
      static {
         TYPE = new ApplyBonusCount.FormulaType(ResourceLocation.withDefaultNamespace("binomial_with_bonus_count"), CODEC);
      }
   }

   private static record FormulaType(ResourceLocation id, Codec<? extends ApplyBonusCount.Formula> codec) {
      FormulaType(ResourceLocation param1, Codec<? extends ApplyBonusCount.Formula> param2) {
         super();
         this.id = var1;
         this.codec = var2;
      }
