...
public class EnderDragon extends Mob implements Enemy {
   ...
   public void aiStep() {
      ...
      if (this.isDeadOrDying()) {
         ...
         this.level().addParticle(ParticleTypes.EXPLOSION, this.getX() + (double)var25, this.getY() + 2.0D + (double)var27, this.getZ() + (double)var31, 0.0D, 0.0D, 0.0D);
      } else {
         this.checkCrystals();
         ...
         if (this.isNoAi()) {
            ...
            if (var37 instanceof ServerLevel) {
               ...
               if (this.hurtTime == 0) {
                  this.knockBack(var10, var10.getEntities(this, this.wing1.getBoundingBox().inflate(4.0D, 2.0D, 4.0D).move(0.0D, -2.0D, 0.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.knockBack(var10, var10.getEntities(this, this.wing2.getBoundingBox().inflate(4.0D, 2.0D, 4.0D).move(0.0D, -2.0D, 0.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.hurt(var10.getEntities(this, this.head.getBoundingBox().inflate(1.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
                  this.hurt(var10.getEntities(this, this.neck.getBoundingBox().inflate(1.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
               }
            }
            ...
            for(var40 = 0; var40 < 3; ++var40) {
               ...
               this.tickPart(var15, (double)(-(var8 * 1.5F + var18 * var43) * var33), var41[1] - var39[1] - (double)((var43 + 1.5F) * var6) + 1.5D, (double)((var35 * 1.5F + var19 * var43) * var33));
            }
   ...
   private void knockBack(ServerLevel var1, List<Entity> var2) {
      double var3 = (this.body.getBoundingBox().minX + this.body.getBoundingBox().maxX) / 2.0D;
      double var5 = (this.body.getBoundingBox().minZ + this.body.getBoundingBox().maxZ) / 2.0D;
      ...
      while(var7.hasNext()) {
         ...
         if (var8 instanceof LivingEntity) {
            ...
            double var14 = Math.max(var10 * var10 + var12 * var12, 0.1D);
            var8.push(var10 / var14 * 4.0D, 0.20000000298023224D, var12 / var14 * 4.0D);
