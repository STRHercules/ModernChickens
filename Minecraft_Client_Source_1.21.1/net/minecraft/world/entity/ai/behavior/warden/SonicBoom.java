      ...
      if (!var2.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY) && !var2.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
         ...
         var10000.filter(var2::canTargetEntity).filter((var1x) -> {
            ...
            for(int var7 = 1; var7 < var6; ++var7) {
               ...
               var1.sendParticles(ParticleTypes.SONIC_BOOM, var8.x, var8.y, var8.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            }

            var2.playSound(SoundEvents.WARDEN_SONIC_BOOM, 3.0F, 1.0F);
            if (var2x.hurt(var1.damageSources().sonicBoom(var2), 10.0F)) {
               double var11 = 0.5D * (1.0D - var2x.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
               double var9 = 2.5D * (1.0D - var2x.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
