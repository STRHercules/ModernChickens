...
public class Breeze extends Monster {
   ...
   public void emitJumpTrailParticles() {
      if (++this.jumpTrailStartedTick <= 5) {
         ...
         Vec3 var3 = this.position().add(var2).add(0.0D, 0.10000000149011612D, 0.0D);

         for(int var4 = 0; var4 < 3; ++var4) {
            this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, var1), var3.x, var3.y, var3.z, 0.0D, 0.0D, 0.0D);
         }
      ...
      if (!this.isPassenger()) {
         ...
         if (var4.getRenderShape() != RenderShape.INVISIBLE) {
            for(int var5 = 0; var5 < var1; ++var5) {
               this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, var4), var3.x, var3.y, var3.z, 0.0D, 0.0D, 0.0D);
            }
   ...
   public boolean withinInnerCircleRange(Vec3 var1) {
      ...
      return var1.closerThan(var2, 4.0D, 10.0D);
   }
