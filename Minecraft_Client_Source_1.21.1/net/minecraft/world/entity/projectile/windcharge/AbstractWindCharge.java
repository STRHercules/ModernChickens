...
public abstract class AbstractWindCharge extends AbstractHurtingProjectile implements ItemSupplier {
   ...
   protected AABB makeBoundingBox() {
      ...
      float var3 = 0.15F;
      return new AABB(this.position().x - (double)var1, this.position().y - 0.15000000596046448D, this.position().z - (double)var1, this.position().x + (double)var1, this.position().y - 0.15000000596046448D + (double)var2, this.position().z + (double)var1);
   }
   ...
   protected void onHitBlock(BlockHitResult var1) {
      ...
      if (!this.level().isClientSide) {
         ...
         Vec3 var3 = Vec3.atLowerCornerOf(var2).multiply(0.25D, 0.25D, 0.25D);
