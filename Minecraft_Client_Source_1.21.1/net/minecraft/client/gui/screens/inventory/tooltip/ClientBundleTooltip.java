...
public class ClientBundleTooltip implements ClientTooltipComponent {
   ...
   private int gridSizeX() {
      return Math.max(2, (int)Math.ceil(Math.sqrt((double)this.contents.size() + 1.0D)));
   }

   private int gridSizeY() {
      return (int)Math.ceil(((double)this.contents.size() + 1.0D) / (double)this.gridSizeX());
   }
   ...
   static enum Texture {
      ...
      private static ClientBundleTooltip.Texture[] $values() {
         return new ClientBundleTooltip.Texture[]{BLOCKED_SLOT, SLOT};
      }
   }
}
