...
public class BandwidthDebugChart extends AbstractDebugChart {
   ...
   protected void renderAdditionalLinesAndLabels(GuiGraphics var1, int var2, int var3, int var4) {
      ...
      this.drawStringWithShade(var1, toDisplayStringInternal(1048576.0D), var2 + 1, var4 - getSampleHeightInternal(1048576.0D) + 1);
   }
   ...
   private static String toDisplayStringInternal(double var0) {
      if (var0 >= 1048576.0D) {
         return String.format(Locale.ROOT, "%.1f MiB/s", var0 / 1048576.0D);
      } else {
         return var0 >= 1024.0D ? String.format(Locale.ROOT, "%.1f KiB/s", var0 / 1024.0D) : String.format(Locale.ROOT, "%d B/s", Mth.floor(var0));
      }
   }
   ...
   private static int getSampleHeightInternal(double var0) {
      return (int)Math.round(Math.log(var0 + 1.0D) * 60.0D / Math.log(1048576.0D));
   }

   protected int getSampleColor(long var1) {
      return this.getSampleColor(toBytesPerSecond((double)var1), 0.0D, -16711681, 8192.0D, -6250241, 1.048576E7D, -65536);
   }
